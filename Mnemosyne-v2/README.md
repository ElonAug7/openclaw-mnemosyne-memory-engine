...
```bash
# 把 Mnemosyne-v2 文件夹复制到工作区 tools/ 下
cp -r Mnemosyne-v2 ~/.openclaw/workspace/tools/
cd ~/.openclaw/workspace/tools/Mnemosyne-v2
bash install.sh
```
...
```bash
# 无需手动复制，脚本自动部署到 workspace/tools/memory-engine
bash /path/to/Mnemosyne-v2/install.sh
```
